...
export class FleetService {
  ...
  async addTruck(companyId: string, userId: string, dto: CreateTruckDto) {
    return this.db.withTenant(companyId, async (c) => {
      ...
      const { rows } = await c.query(
        ...
        [companyId, dto.regNumber, dto.truckType, dto.capacityKg, dto.gpsDeviceId ?? null]
      );
  ...
  async listTrucks(companyId: string, status?: string) {
    return this.db.withTenant(companyId, async (c) => {
      const { rows } = status
  ...
  async setTruckStatus(companyId: string, id: string, status: string) {
    return this.db.withTenant(companyId, async (c) => {
      const { rows } = await c.query(
  ...
  async addDriver(companyId: string, userId: string, dto: CreateDriverDto) {
    return this.db.withTenant(companyId, async (c) => {
      ...
      const { rows } = await c.query(
        ...
        [companyId, dto.fullName, dto.phone, dto.licenseNumber]
      );
      await this.audit.record({
        companyId, userId,
  ...
  async availability(companyId: string) {
    return this.db.withTenant(companyId, async (c) => {
