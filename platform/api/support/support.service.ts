...
export class SupportService {
  ...
  async createTicket(companyId: string, userId: string, dto: CreateTicketDto) {
    return this.db.withTenant(companyId, async (c) => {
      const { rows } = await c.query(
        ...
        [companyId, dto.bookingId ?? null, dto.category, dto.subject, userId]
      );
      await c.query(
  ...
  async listTickets(companyId: string) {
    return this.db.withTenant(companyId, async (c) => {
      const { rows } = await c.query(
  ...
  async reply(companyId: string, userId: string, ticketId: string, message: string) {
    return this.db.withTenant(companyId, async (c) => {
  ...
  async callDriver(companyId: string, userId: string, bookingId: string) {
    return this.db.withTenant(companyId, async (c) => {
      const { rows } = await c.query(
